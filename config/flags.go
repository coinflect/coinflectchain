...
import (
	...
	"github.com/coinflect/coinflectchain/genesis"
	"github.com/coinflect/coinflectchain/trace"
...
const (
	...
	CoinflectChainDataDirVar    = "COINFLECTCHAIN_DATA_DIR"
	defaultUnexpandedDataDir = "$" + CoinflectChainDataDirVar
)

var (
	// [defaultUnexpandedDataDir] will be expanded when reading the flags
	defaultDataDir              = filepath.Join("$HOME", ".coinflectchain")
...
func addNodeFlags(fs *flag.FlagSet) {
	...
	// CFLT fees
	fs.Uint64(TxFeeKey, genesis.LocalParams.TxFee, "Transaction fee, in nCFLT")
	fs.Uint64(CreateAssetTxFeeKey, genesis.LocalParams.CreateAssetTxFee, "Transaction fee, in nCFLT, for transactions that create new assets")
	fs.Uint64(CreateSubnetTxFeeKey, genesis.LocalParams.CreateSubnetTxFee, "Transaction fee, in nCFLT, for transactions that create new subnets")
	fs.Uint64(TransformSubnetTxFeeKey, genesis.LocalParams.TransformSubnetTxFee, "Transaction fee, in nCFLT, for transactions that transform subnets")
	fs.Uint64(CreateBlockchainTxFeeKey, genesis.LocalParams.CreateBlockchainTxFee, "Transaction fee, in nCFLT, for transactions that create new blockchains")
	fs.Uint64(AddPrimaryNetworkValidatorFeeKey, genesis.LocalParams.AddPrimaryNetworkValidatorFee, "Transaction fee, in nCFLT, for transactions that add new primary network validators")
	fs.Uint64(AddPrimaryNetworkDelegatorFeeKey, genesis.LocalParams.AddPrimaryNetworkDelegatorFee, "Transaction fee, in nCFLT, for transactions that add new primary network delegators")
	fs.Uint64(AddSubnetValidatorFeeKey, genesis.LocalParams.AddSubnetValidatorFee, "Transaction fee, in nCFLT, for transactions that add new subnet validators")
	fs.Uint64(AddSubnetDelegatorFeeKey, genesis.LocalParams.AddSubnetDelegatorFee, "Transaction fee, in nCFLT, for transactions that add new subnet delegators")
	...
	// Logging
	fs.String(LogsDirKey, defaultLogDir, "Logging directory for Coinflect")
	...
	fs.String(HTTPAllowedOrigins, "*", "Origins to allow on the HTTP port. Defaults to * which allows all origins. Example: https://*.coinflect.com")
	...
	fs.Uint64(MinValidatorStakeKey, genesis.LocalParams.MinValidatorStake, "Minimum stake, in nCFLT, required to validate the primary network")
	// Maximum Stake that can be staked and delegated to a validator on the Primary Network
	fs.Uint64(MaxValidatorStakeKey, genesis.LocalParams.MaxValidatorStake, "Maximum stake, in nCFLT, that can be placed on a validator on the primary network")
	// Minimum Stake that can be delegated on the Primary Network
	fs.Uint64(MinDelegatorStakeKey, genesis.LocalParams.MinDelegatorStake, "Minimum stake, in nCFLT, that can be delegated on the primary network")
	...
	fs.Int(SnowCoinflectNumParentsKey, 5, "Number of vertexes for reference from each new vertex")
	fs.Int(SnowCoinflectBatchSizeKey, 30, "Number of operations to batch in each new vertex")
...
// BuildFlagSet returns a complete set of flags for coinflectchain
...
// any variables using the OS env. If the [CoinflectChainDataDirVar] var is used,
...
// [CoinflectChainDataDirVar] var is used, we expand the value of the variable with
...
func GetExpandedString(v *viper.Viper, s string) string {
	return os.Expand(
		...
		func(strVar string) string {
			if strVar == CoinflectChainDataDirVar {
