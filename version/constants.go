...
// the plugin vm to upgrade to latest coinflectchain release to be compatible.
...
var (
	...
	CurrentDatabase = DatabaseVersion1_0_0
	...
	// set of coinflectchain versions that supported that version. This is not used
	// by coinflectchain, but is useful for downstream libraries.
	RPCChainVMProtocolCompatibility map[uint][]*Semantic

	ApricotPhase3Times = map[uint32]time.Time{
		constants.MainnetID: time.Date(2021, time.August, 24, 14, 0, 0, 0, time.UTC),
		constants.AddismyaID:    time.Date(2021, time.August, 16, 19, 0, 0, 0, time.UTC),
	}
	...
	ApricotPhase6Times = map[uint32]time.Time{
		...
		constants.AddismyaID:    time.Date(2022, time.September, 6, 20, 0, 0, 0, time.UTC),
	}
	...
	BanffTimes = map[uint32]time.Time{
		...
		constants.AddismyaID:    time.Date(2022, time.October, 3, 14, 0, 0, 0, time.UTC),
	}
	...
	XChainMigrationTimes = map[uint32]time.Time{
		...
		constants.AddismyaID:    time.Date(10000, time.December, 1, 0, 0, 0, 0, time.UTC),
	}
