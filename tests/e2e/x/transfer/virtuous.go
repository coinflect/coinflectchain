...
import (
	...
	"github.com/coinflect/coinflectchain/tests/e2e"
	...
	"github.com/coinflect/coinflectchain/wallet/subnet/primary"
...
const (
	metricVtxProcessing = "coinflect_X_vtx_processing"
	metricVtxAccepted   = "coinflect_X_vtx_accepted_count"
	metricVtxRejected   = "coinflect_X_vtx_rejected_count"
)

const totalRounds = 50

var _ = e2e.DescribeXChain("[Virtuous Transfer Tx CFLT]", func() {
	ginkgo.It("can issue a virtuous transfer tx for CFLT asset",
		// use this for filtering tests by labels
		// ref. https://onsi.github.io/ginkgo/#spec-labels
		ginkgo.Label(
			"require-network-runner",
			"x",
			"virtuous-transfer-tx-cflt",
		),
		func() {
			...
			runFunc := func(round int) {
				...
				cfltAssetID := baseWallet.X().CFLTAssetID()
				...
				for i, w := range wallets {
					...
					bal := balances[cfltAssetID]
					testBalances = append(testBalances, bal)

					fmt.Printf(`CURRENT BALANCE %21d CFLT (SHORT ADDRESS %q)
`,
						bal,
...
FROM [%q]
SENDER    CURRENT BALANCE     : %21d CFLT
SENDER    NEW BALANCE (AFTER) : %21d CFLT

TRANSFER AMOUNT FROM SENDER   : %21d CFLT

TO [%q]
RECEIVER  CURRENT BALANCE     : %21d CFLT
RECEIVER  NEW BALANCE (AFTER) : %21d CFLT
===
`,
					shortAddrs[fromIdx],
					senderOrigBal,
