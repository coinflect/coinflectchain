...
type testEnvironmentConfig struct {
	...
	CoinflectChainExecPath       string
	CoinflectChainLogLevel       string
	testKeysFile              string
...
func (te *TestEnvinronment) ConfigCluster(
	...
	if CoinflectChainExecPath != "" {
		if _, err := os.Stat(CoinflectChainExecPath); err != nil {
			return fmt.Errorf("could not find coinflectchain binary: %w", err)
		}
	}
	...
	case networkRunnerGRPCEp != "" && len(uris) == 0:
		...
		te.CoinflectChainExecPath = CoinflectChainExecPath
		te.CoinflectChainLogLevel = CoinflectChainLogLevel
...
func (te *TestEnvinronment) StartCluster() error {
	...
	case StandAlone:
		tests.Outf("{{magenta}}starting network-runner with %q{{/}}\n", te.CoinflectChainExecPath)
		ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
		resp, err := te.GetRunnerClient().Start(ctx, te.CoinflectChainExecPath,
			runner_sdk.WithNumNodes(5),
			runner_sdk.WithGlobalNodeConfig(fmt.Sprintf(`{"log-level":"%s"}`, te.CoinflectChainLogLevel)),
		)
		cancel()
		if err != nil {
