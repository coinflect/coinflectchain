...
import (
	...
	"github.com/coinflect/coinflectchain/snow/consensus/coinflect/poll"
...
func (t *Transitive) Start(ctx context.Context, startReqID uint32) error {
	...
	edge := t.Manager.Edge(ctx)
	frontier := make([]coinflect.Vertex, 0, len(edge))
...
func (t *Transitive) GetVtx(ctx context.Context, vtxID ids.ID) (coinflect.Vertex, error) {
...
// Returns true if [vtx] has been added to consensus (now or previously)
func (t *Transitive) issueFrom(ctx context.Context, nodeID ids.NodeID, vtx coinflect.Vertex) (bool, error) {
	issued := true
...
// Assumes we have [vtx].
func (t *Transitive) issue(ctx context.Context, vtx coinflect.Vertex) error {
	vtxID := vtx.ID()
