...
var (
	...
	_ coinflect.Vertex = (*uniqueVertex)(nil)
)
...
func (vtx *uniqueVertex) Parents() ([]coinflect.Vertex, error) {
	vtx.refresh()
	...
	if len(vtx.v.parents) != len(parentIDs) {
		vtx.v.parents = make([]coinflect.Vertex, len(parentIDs))
