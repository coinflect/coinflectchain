...
import (
	...
	"github.com/coinflect/coinflectchain/snow/consensus/metrics"
...
// Topological performs the coinflect algorithm by utilizing a topological sort
...
func (ta *Topological) HealthCheck(ctx context.Context) (interface{}, error) {
	...
	if !healthy {
		...
		return details, fmt.Errorf("coinflect consensus is not healthy reason: %s", strings.Join(errorReasons, ", "))
	}
	return details, nil
}
