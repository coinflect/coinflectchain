...
type Client interface {
	...
	// ExportCFLT issues an ExportTx transaction and returns the txID
	ExportCFLT(
		ctx context.Context,
		user api.UserPass,
		from []ids.ShortID,
	...
	// ImportCFLT issues an ImportTx transaction and returns the txID
	ImportCFLT(
		ctx context.Context,
		user api.UserPass,
		from []ids.ShortID,
...
func (c *client) ExportCFLT(
	...
	res := &api.JSONTxID{}
	err := c.requester.SendRequest(ctx, "platform.ExportCFLT", &ExportCFLTArgs{
		JSONSpendHeader: api.JSONSpendHeader{
			UserPass:       user,
			JSONFromAddrs:  api.JSONFromAddrs{From: ids.ShortIDsToStrings(from)},
			JSONChangeAddr: api.JSONChangeAddr{ChangeAddr: changeAddr.String()},
...
func (c *client) ImportCFLT(
	...
	res := &api.JSONTxID{}
	err := c.requester.SendRequest(ctx, "platform.importCFLT", &ImportCFLTArgs{
		JSONSpendHeader: api.JSONSpendHeader{
			UserPass:       user,
			JSONFromAddrs:  api.JSONFromAddrs{From: ids.ShortIDsToStrings(from)},
			JSONChangeAddr: api.JSONChangeAddr{ChangeAddr: changeAddr.String()},
