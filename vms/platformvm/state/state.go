...
import (
	...
	"github.com/coinflect/coinflectchain/cache/metercacher"
	"github.com/coinflect/coinflectchain/database"
	"github.com/coinflect/coinflectchain/database/linkeddb"
...
type Chain interface {
	...
	GetRewardUTXOs(txID ids.ID) ([]*cflt.UTXO, error)
	AddRewardUTXO(txID ids.ID, utxo *cflt.UTXO)
...
type state struct {
	...
	addedRewardUTXOs map[ids.ID][]*cflt.UTXO // map of txID -> []*UTXO
	rewardUTXOsCache cache.Cacher            // cache of txID -> []*UTXO
	rewardUTXODB     database.Database

	modifiedUTXOs map[ids.ID]*cflt.UTXO // map of modified UTXOID -> *UTXO if the UTXO is nil, it has been removed
	utxoDB        database.Database
	utxoState     cflt.UTXOState
...
func new(
	...
	utxoState, err := cflt.NewMeteredUTXOState(utxoDB, txs.GenesisCodec, metricsReg)
	if err != nil {
		return nil, err
	}
	...
	return &state{
		...
		addedRewardUTXOs: make(map[ids.ID][]*cflt.UTXO),
		rewardUTXODB:     rewardUTXODB,
		rewardUTXOsCache: rewardUTXOsCache,

		modifiedUTXOs: make(map[ids.ID]*cflt.UTXO),
		utxoDB:        utxoDB,
		utxoState:     utxoState,
...
func (s *state) GetRewardUTXOs(txID ids.ID) ([]*cflt.UTXO, error) {
	if utxos, exists := s.addedRewardUTXOs[txID]; exists {
		return utxos, nil
	}
	if utxos, exists := s.rewardUTXOsCache.Get(txID); exists {
		return utxos.([]*cflt.UTXO), nil
	}
...
func (s *state) AddRewardUTXO(txID ids.ID, utxo *cflt.UTXO) {
	s.addedRewardUTXOs[txID] = append(s.addedRewardUTXOs[txID], utxo)
}

func (s *state) GetUTXO(utxoID ids.ID) (*cflt.UTXO, error) {
...
func (s *state) AddUTXO(utxo *cflt.UTXO) {
