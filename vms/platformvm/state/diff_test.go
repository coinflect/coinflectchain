...
func TestDiffRewardUTXO(t *testing.T) {
	...
	// Put a reward UTXO
	txID := ids.GenerateTestID()
	rewardUTXO := &cflt.UTXO{
		UTXOID: cflt.UTXOID{TxID: txID},
	}
...

	{
		...
		parentRewardUTXO := &cflt.UTXO{
			UTXOID: cflt.UTXOID{TxID: txID2},
		}
		state.EXPECT().GetRewardUTXOs(txID2).Return([]*cflt.UTXO{parentRewardUTXO}, nil).Times(1)
