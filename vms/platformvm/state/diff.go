...
type diff struct {
	...
	// map of txID -> []*UTXO
	addedRewardUTXOs map[ids.ID][]*cflt.UTXO
...
func (d *diff) GetRewardUTXOs(txID ids.ID) ([]*cflt.UTXO, error) {
...
func (d *diff) AddRewardUTXO(txID ids.ID, utxo *cflt.UTXO) {
	if d.addedRewardUTXOs == nil {
		d.addedRewardUTXOs = make(map[ids.ID][]*cflt.UTXO)
	}
	d.addedRewardUTXOs[txID] = append(d.addedRewardUTXOs[txID], utxo)
}

func (d *diff) GetUTXO(utxoID ids.ID) (*cflt.UTXO, error) {
...
func (d *diff) AddUTXO(utxo *cflt.UTXO) {
