...
import (
	...
	"github.com/coinflect/coinflectchain/snow/engine/snowman/bootstrap"
	...
	p2ppb "github.com/coinflect/coinflectchain/proto/pb/p2p"
...
var (
	...
	// each key controls an address that has [defaultBalance] CFLT at genesis
	keys = crypto.BuildTestKeys()

	defaultMinValidatorStake = 5 * units.MilliCflt
	defaultMaxValidatorStake = 500 * units.MilliCflt
	defaultMinDelegatorStake = 1 * units.MilliCflt
...
func TestGenesis(t *testing.T) {
	...
	for _, utxo := range genesisState.UTXOs {
		...
		addrs := ids.ShortSet{}
		addrs.Add(addr)
		utxos, err := cflt.GetAllUTXOs(vm.state, addrs)
		if err != nil {
...
func TestRestartFullyAccepted(t *testing.T) {
	...
	tx := &txs.Tx{Unsigned: &txs.ImportTx{
		...
		ImportedInputs: []*cflt.TransferableInput{{
			...
			Asset: cflt.Asset{ID: firstVM.ctx.CFLTAssetID},
			In: &secp256k1fx.TransferInput{
				Amt: 50000,
			},
		}},
