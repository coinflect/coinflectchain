...
import (
	...
	vmkeystore "github.com/coinflect/coinflectchain/vms/components/keystore"
	pchainapi "github.com/coinflect/coinflectchain/vms/platformvm/api"
...
func TestGetStake(t *testing.T) {
	...
	for i, validator := range genesis.Validators {
		...
		var output cflt.TransferableOutput
	...
	outputs := make([]cflt.TransferableOutput, 2)
	for i := range outputs {
	...
	// Unmarshal
	outputs = make([]cflt.TransferableOutput, 3)
	for i := range outputs {
		outputBytes, err := formatting.Decode(args.Encoding, response.Outputs[i])
		require.NoError(err)
		_, err = txs.Codec.Unmarshal(outputBytes, &outputs[i])
