...
import (
	...
	"github.com/coinflect/coinflectchain/vms/components/keystore"
	...
	"github.com/coinflect/coinflectchain/vms/platformvm/txs/builder"
	"github.com/coinflect/coinflectchain/vms/platformvm/txs/executor"
	"github.com/coinflect/coinflectchain/vms/secp256k1fx"

	platformapi "github.com/coinflect/coinflectchain/vms/platformvm/api"
)
...
type Service struct {
	vm          *VM
	addrManager cflt.AddressManager
}
...
func (service *Service) ExportKey(_ *http.Request, args *ExportKeyArgs, reply *ExportKeyReply) error {
	...
	address, err := cflt.ParseServiceAddress(service.addrManager, args.Address)
	if err != nil {
...
// Note: We explicitly duplicate CFLT out of the maps to ensure backwards
// compatibility.
type GetBalanceResponse struct {
	// Balance, in nCFLT, of the address
	...
	UTXOIDs             []*cflt.UTXOID         `json:"utxoIDs"`
}
...
func (service *Service) GetBalance(_ *http.Request, args *GetBalanceRequest, response *GetBalanceResponse) error {
	...
	// Parse to address
	addrs, err := cflt.ParseServiceAddresses(service.addrManager, args.Addresses)
	if err != nil {
		return err
	}

	utxos, err := cflt.GetAllUTXOs(service.vm.state, addrs)
	if err != nil {
...
utxoFor:
	...
	response.Balance = response.Balances[service.vm.ctx.CFLTAssetID]
	response.Unlocked = response.Unlockeds[service.vm.ctx.CFLTAssetID]
	response.LockedStakeable = response.LockedStakeables[service.vm.ctx.CFLTAssetID]
	response.LockedNotStakeable = response.LockedNotStakeables[service.vm.ctx.CFLTAssetID]
	return nil
}
...
func (service *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, response *api.GetUTXOsReply) error {
	...
	addrSet, err := cflt.ParseServiceAddresses(service.addrManager, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = cflt.ParseServiceAddress(service.addrManager, args.StartIndex.Address)
		if err != nil {
	...
	if sourceChain == service.vm.ctx.ChainID {
		utxos, endAddr, endUTXOID, err = cflt.GetPaginatedUTXOs(
			service.vm.state,
			addrSet,
			startAddr,
			startUTXO,
...
func (service *Service) GetStakingAssetID(_ *http.Request, args *GetStakingAssetIDArgs, response *GetStakingAssetIDResponse) error {
	...
	if args.SubnetID == constants.PrimaryNetworkID {
		response.AssetID = service.vm.ctx.CFLTAssetID
		return nil
	}
...
// GetCurrentSupply returns an upper bound on the supply of CFLT in the system
...
func (service *Service) AddValidator(_ *http.Request, args *AddValidatorArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	fromAddrs, err := cflt.ParseServiceAddresses(service.addrManager, args.From)
	if err != nil {
		return err
	}

	// Parse the reward address
	rewardAddress, err := cflt.ParseServiceAddress(service.addrManager, args.RewardAddress)
	if err != nil {
	...
	if args.ChangeAddr != "" {
		changeAddr, err = cflt.ParseServiceAddress(service.addrManager, args.ChangeAddr)
		if err != nil {
...
func (service *Service) CreateSubnet(_ *http.Request, args *CreateSubnetArgs, response *api.JSONTxIDChangeAddr) error {
	...
	controlKeys, err := cflt.ParseServiceAddresses(service.addrManager, args.ControlKeys)
	if err != nil {
		return err
	}
...
type ExportCFLTArgs struct {
	...
	// Amount of CFLT to send
	Amount json.Uint64 `json:"amount"`

	// Chain the funds are going to. Optional. Used if To address does not include the chainID.
	TargetChain string `json:"targetChain"`

	// ID of the address that will receive the cflt. This address may include the
	// chainID, which is used to determine what the destination chain is.
	To string `json:"to"`
}

// ExportCFLT exports CFLT from the P-Chain to the X-Chain
// It must be imported on the X-Chain to complete the transfer
func (service *Service) ExportCFLT(_ *http.Request, args *ExportCFLTArgs, response *api.JSONTxIDChangeAddr) error {
	service.vm.ctx.Log.Debug("Platform: ExportCFLT called")
...
// ImportCFLTArgs are the arguments to ImportCFLT
type ImportCFLTArgs struct {
	// User, password, from addrs, change addr
	api.JSONSpendHeader
...
// ImportCFLT issues a transaction to import CFLT from the X-chain. The CFLT
// must have already been exported from the X-Chain.
func (service *Service) ImportCFLT(_ *http.Request, args *ImportCFLTArgs, response *api.JSONTxIDChangeAddr) error {
	service.vm.ctx.Log.Debug("Platform: ImportCFLT called")
	...
	to, err := cflt.ParseServiceAddress(service.addrManager, args.To)
...
type GetStakeReply struct {
	...
	// Each is of type cflt.TransferableOutput
	Outputs []string `json:"stakedOutputs"`
	// Encoding of [Outputs]
	Encoding formatting.Encoding `json:"encoding"`
}

// GetStake returns the amount of nCFLT that [args.Addresses] have cumulatively
// staked on the Primary Network.
//
// This method assumes that each stake output has only owner
// This method assumes only CFLT can be staked
...
func (service *Service) GetStake(_ *http.Request, args *GetStakeArgs, response *GetStakeReply) error {
	...
	var (
		...
		stakedOuts        []cflt.TransferableOutput
	)
	...
	response.Staked = response.Stakeds[service.vm.ctx.CFLTAssetID]
...
type GetMinStakeReply struct {
	...
	// Minimum stake, in nCFLT, that can be delegated on the primary network
	MinDelegatorStake json.Uint64 `json:"minDelegatorStake"`
}

// GetMinStake returns the minimum staking amount in ncflt.
...
// GetMaxStakeAmount returns the maximum amount of nCFLT staking to the named
...
func getStakeHelper(tx *txs.Tx, addrs ids.ShortSet, totalAmountStaked map[ids.ID]uint64) []cflt.TransferableOutput {
	...
	stake := staker.Stake()
	stakedOuts := make([]cflt.TransferableOutput, 0, len(stake))
