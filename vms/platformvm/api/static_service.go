...
import (
	...
	"github.com/coinflect/coinflectchain/vms/platformvm/txs/txheap"
	"github.com/coinflect/coinflectchain/vms/platformvm/validator"
	"github.com/coinflect/coinflectchain/vms/secp256k1fx"
)

// Note that since an Coinflect network has exactly one Platform Chain,
...
type BuildGenesisArgs struct {
	CfltAssetID   ids.ID                    `json:"cfltAssetID"`
...
// BuildGenesis build the genesis state of the Platform Chain (and thereby the Coinflect network.)
func (*StaticService) BuildGenesis(_ *http.Request, args *BuildGenesisArgs, reply *BuildGenesisReply) error {
	...
	for i, apiUTXO := range args.UTXOs {
		...
		utxo := cflt.UTXO{
			...
			Asset: cflt.Asset{ID: args.CfltAssetID},
		...
		if apiUTXO.Locktime > args.Time {
			utxo.Out = &stakeable.LockOut{
				...
				TransferableOut: utxo.Out.(cflt.TransferableOut),
			}
		}
	...
	for _, vdr := range args.Validators {
		weight := uint64(0)
		stake := make([]*cflt.TransferableOutput, len(vdr.Staked))
