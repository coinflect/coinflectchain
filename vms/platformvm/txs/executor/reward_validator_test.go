...
func TestRewardValidatorTxExecuteOnCommit(t *testing.T) {
	...
	// Get old balances
	oldBalance, err := cflt.GetBalance(env.state, stakeOwners)
	require.NoError(err)
	...
	onCommitBalance, err := cflt.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardValidatorTxExecuteOnAbort(t *testing.T) {
	...
	onAbortBalance, err := cflt.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardDelegatorTxExecuteOnCommit(t *testing.T) {
	...
	oldVdrBalance, err := cflt.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	oldDelBalance, err := cflt.GetBalance(env.state, delDestSet)
	require.NoError(err)
	...
	commitVdrBalance, err := cflt.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	...
	commitDelBalance, err := cflt.GetBalance(env.state, delDestSet)
	require.NoError(err)
