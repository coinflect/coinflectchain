...
func (e *StandardTxExecutor) CreateChainTx(tx *txs.CreateChainTx) error {
	...
	if err := e.FlowChecker.VerifySpend(
		...
		baseTxCreds,
		map[ids.ID]uint64{
			e.Ctx.CFLTAssetID: createBlockchainTxFee,
		},
	); err != nil {
		return err
	}
...
func (e *StandardTxExecutor) CreateSubnetTx(tx *txs.CreateSubnetTx) error {
	...
	if err := e.FlowChecker.VerifySpend(
		...
		e.Tx.Creds,
		map[ids.ID]uint64{
			e.Ctx.CFLTAssetID: createSubnetTxFee,
		},
	); err != nil {
		return err
	}
...
func (e *StandardTxExecutor) ImportTx(tx *txs.ImportTx) error {
	...
	if e.Bootstrapped.GetValue() {
		...
		utxos := make([]*cflt.UTXO, len(tx.Ins)+len(tx.ImportedInputs))
		...
		ins := make([]*cflt.TransferableInput, len(tx.Ins)+len(tx.ImportedInputs))
		copy(ins, tx.Ins)
		...
		if err := e.FlowChecker.VerifySpendUTXOs(
			...
			e.Tx.Creds,
			map[ids.ID]uint64{
				e.Ctx.CFLTAssetID: e.Config.TxFee,
			},
		); err != nil {
			return err
		}
...
func (e *StandardTxExecutor) ExportTx(tx *txs.ExportTx) error {
	...
	outs := make([]*cflt.TransferableOutput, len(tx.Outs)+len(tx.ExportedOutputs))
	copy(outs, tx.Outs)
...
func (e *StandardTxExecutor) TransformSubnetTx(tx *txs.TransformSubnetTx) error {
	...
	if err := e.Backend.FlowChecker.VerifySpend(
		...
		tx.Outs,
		baseTxCreds,
		// Invariant: [tx.AssetID != e.Ctx.CFLTAssetID]. This prevents the first
		//            entry in this map literal from being overwritten by the
		//            second entry.
		map[ids.ID]uint64{
			e.Ctx.CFLTAssetID: e.Config.TransformSubnetTxFee,
