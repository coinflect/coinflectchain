...
func Consume(utxoDB state.UTXODeleter, ins []*cflt.TransferableInput) {
...
type Spender interface {
	...
	// - [fee] is the amount of CFLT that should be burned
	...
	Spend(
		...
		[]*cflt.TransferableInput, // inputs
		[]*cflt.TransferableOutput, // returnedOutputs
		[]*cflt.TransferableOutput, // stakedOutputs
...
func (h *handler) Spend(
	...
	utxos, err := cflt.GetAllUTXOs(h.utxosReader, addrs) // The UTXOs controlled by [keys]
	if err != nil {
	...
	ins := []*cflt.TransferableInput{}
	returnedOuts := []*cflt.TransferableOutput{}
	stakedOuts := []*cflt.TransferableOutput{}
	signers := [][]*crypto.PrivateKeySECP256K1R{}

	// Amount of CFLT that has been staked
	amountStaked := uint64(0)

	// Consume locked UTXOs
	for _, utxo := range utxos {
		// If we have consumed more CFLT than we are trying to stake, then we
		// have no need to consume more locked CFLT
		if amountStaked >= amount {
			break
		}

		if assetID := utxo.AssetID(); assetID != h.ctx.CFLTAssetID {
			continue // We only care about staking CFLT, so ignore other assets
		}
		...
		in, ok := inIntf.(cflt.TransferableIn)
		if !ok { // should never happen
			h.ctx.Log.Warn("wrong input type",
				zap.String("expectedType", "cflt.TransferableIn"),
		...
		ins = append(ins, &cflt.TransferableInput{
			UTXOID: utxo.UTXOID,
		...
		stakedOuts = append(stakedOuts, &cflt.TransferableOutput{
			Asset: cflt.Asset{ID: h.ctx.CFLTAssetID},
		...
		if remainingValue > 0 {
			...
			returnedOuts = append(returnedOuts, &cflt.TransferableOutput{
				Asset: cflt.Asset{ID: h.ctx.CFLTAssetID},
				Out: &stakeable.LockOut{
					Locktime: out.Locktime,
					TransferableOut: &secp256k1fx.TransferOutput{
	...
	// Amount of CFLT that has been burned
	amountBurned := uint64(0)

	for _, utxo := range utxos {
		// If we have consumed more CFLT than we are trying to stake,
		// and we have burned more CFLT than we need to,
		// then we have no need to consume more CFLT
		if amountBurned >= fee && amountStaked >= amount {
			break
		}

		if assetID := utxo.AssetID(); assetID != h.ctx.CFLTAssetID {
			continue // We only care about burning CFLT, so ignore other assets
		}
	...
	cflt.SortTransferableInputsWithSigners(ins, signers)  // sort inputs and keys
	cflt.SortTransferableOutputs(returnedOuts, txs.Codec) // sort outputs
	cflt.SortTransferableOutputs(stakedOuts, txs.Codec)   // sort outputs
