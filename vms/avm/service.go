...
type GetAddressTxsArgs struct {
	...
	// AssetID defaulted to CFLT if omitted or left blank
...
func (service *Service) GetAddressTxs(_ *http.Request, args *GetAddressTxsArgs, reply *GetAddressTxsReply) error {
	...
	// Parse to address
	address, err := cflt.ParseServiceAddress(service.vm, args.Address)
	if err != nil {
...
func (service *Service) GetUTXOs(_ *http.Request, args *api.GetUTXOsArgs, reply *api.GetUTXOsReply) error {
	...
	addrSet, err := cflt.ParseServiceAddresses(service.vm, args.Addresses)
	if err != nil {
		return err
	}
	...
	if args.StartIndex.Address != "" || args.StartIndex.UTXO != "" {
		startAddr, err = cflt.ParseServiceAddress(service.vm, args.StartIndex.Address)
		if err != nil {
...
func (service *Service) GetBalance(_ *http.Request, args *GetBalanceArgs, reply *GetBalanceReply) error {
	...
	addr, err := cflt.ParseServiceAddress(service.vm, args.Address)
	if err != nil {
	...
	utxos, err := cflt.GetAllUTXOs(service.vm.state, addrSet)
	if err != nil {
		return fmt.Errorf("problem retrieving UTXOs: %w", err)
	}

	now := service.vm.clock.Unix()
	reply.UTXOIDs = make([]cflt.UTXOID, 0, len(utxos))
...
func (service *Service) CreateAsset(_ *http.Request, args *CreateAssetArgs, reply *AssetIDChangeAddr) error {
	...
	fromAddrs, err := cflt.ParseServiceAddresses(service.vm, args.From)
	if err != nil {
		return err
	}
	...
	if amountSpent := amountsSpent[service.vm.feeAssetID]; amountSpent > service.vm.CreateAssetTxFee {
		outs = append(outs, &cflt.TransferableOutput{
			Asset: cflt.Asset{ID: service.vm.feeAssetID},
	...
	for _, holder := range args.InitialHolders {
		addr, err := cflt.ParseServiceAddress(service.vm, holder.Address)
		if err != nil {
			return err
		}
	...
	for _, owner := range args.MinterSets {
		...
		minterAddrsSet, err := cflt.ParseServiceAddresses(service.vm, owner.Minters)
		if err != nil {
			return err
		}
...
func (service *Service) SendMultiple(_ *http.Request, args *SendMultipleArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	if l := len(memoBytes); l > cflt.MaxMemoSize {
		return fmt.Errorf("max memo length is %d but provided memo field is length %d", cflt.MaxMemoSize, l)
	...
	for _, output := range args.Outputs {
		...
		to, err := cflt.ParseServiceAddress(service.vm, output.To)
		if err != nil {
	...
	cflt.SortTransferableOutputs(outs, service.vm.parser.Codec())
...
func (service *Service) Mint(_ *http.Request, args *MintArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	to, err := cflt.ParseServiceAddress(service.vm, args.To)
	if err != nil {
...
type ImportArgs struct {
	...
	// Address receiving the imported CFLT
	To string `json:"to"`
}

// Import imports an asset to this chain from the P/C-Chain.
// The CFLT must have already been exported from the P/C-Chain.
...
func (service *Service) Export(_ *http.Request, args *ExportArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	exportOuts := []*cflt.TransferableOutput{{
		Asset: cflt.Asset{ID: assetID},
		Out: &secp256k1fx.TransferOutput{
