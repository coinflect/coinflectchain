...
func (w *WalletService) update(utxos []*cflt.UTXO) ([]*cflt.UTXO, error) {
	utxoMap := make(map[ids.ID]*cflt.UTXO, len(utxos))
	...
	newUTXOs := make([]*cflt.UTXO, len(utxoMap))
	i := 0
...
func (w *WalletService) SendMultiple(_ *http.Request, args *SendMultipleArgs, reply *api.JSONTxIDChangeAddr) error {
	...
	fromAddrs, err := cflt.ParseServiceAddresses(w.vm, args.From)
	if err != nil {
	...
	for _, output := range args.Outputs {
		...
		to, err := cflt.ParseServiceAddress(w.vm, output.To)
		if err != nil {
	...
	cflt.SortTransferableOutputs(outs, codec)
