...
func NewContext(tb testing.TB) *snow.Context {
	...
	ctx.ChainID = chainID
	cfltAssetID, err := ids.FromString("2XGxUr7VF7j1iwUp2aiGe4b6Ue2yyNghNS1SuNTNmZ77dPpXFZ")
	if err != nil {
		tb.Fatal(err)
	}
...
func TestTxInvalidCredential(t *testing.T) {
	...
	tx := &Tx{
		Unsigned: &BaseTx{BaseTx: cflt.BaseTx{
			NetworkID:    networkID,
		...
		Creds: []*fxs.FxCredential{{Verifiable: &cflt.TestVerifiable{Err: errors.New("")}}},
	}
...
func TestTxInvalidNumberOfCredentials(t *testing.T) {
	...
	tx := &Tx{
		Unsigned: &BaseTx{BaseTx: cflt.BaseTx{
			...
			Ins: []*cflt.TransferableInput{
				{
					UTXOID: cflt.UTXOID{TxID: ids.Empty, OutputIndex: 0},
					Asset:  cflt.Asset{ID: assetID},
					In: &secp256k1fx.TransferInput{
						Amt: 20 * units.KiloCflt,
						Input: secp256k1fx.Input{
				...
				{
					UTXOID: cflt.UTXOID{TxID: ids.Empty, OutputIndex: 1},
					Asset:  cflt.Asset{ID: assetID},
					In: &secp256k1fx.TransferInput{
						Amt: 20 * units.KiloCflt,
						Input: secp256k1fx.Input{
		...
		Creds: []*fxs.FxCredential{{Verifiable: &cflt.TestVerifiable{}}},
	}
	tx.Initialize(nil, nil)
