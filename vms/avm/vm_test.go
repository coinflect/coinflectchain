...
import (
	...
	"github.com/coinflect/coinflectchain/database/mockdb"
	...
	"github.com/coinflect/coinflectchain/vms/avm/states"
...
func NewContext(tb testing.TB) *snow.Context {
	...
	tx := GetCFLTTxFromGenesisTest(genesisBytes, tb)

	ctx := snow.DefaultContextTest()
	ctx.NetworkID = networkID
	ctx.ChainID = chainID
	ctx.CFLTAssetID = tx.ID()
...
func GetCFLTTxFromGenesisTest(genesisBytes []byte, tb testing.TB) *txs.Tx {
	return GetCreateTxFromGenesisTest(tb, genesisBytes, "CFLT")
}
...
func NewTx(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	return NewTxWithAsset(t, genesisBytes, vm, "CFLT")
}
	...
	newTx := &txs.Tx{Unsigned: &txs.BaseTx{
		BaseTx: cflt.BaseTx{
			...
			Ins: []*cflt.TransferableInput{{
				...
				Asset: cflt.Asset{ID: createTx.ID()},
...
func setupIssueTx(t testing.TB) (chan common.Message, *VM, *snow.Context, []*txs.Tx) {
	...
	return issuer, vm, ctx, []*txs.Tx{cfltTx, firstTx, secondTx}
}
		...
		BaseTx: cflt.BaseTx{
			...
			Ins: []*cflt.TransferableInput{
				// fee asset
				{
					...
					Asset: cflt.Asset{ID: feeAssetCreateTx.ID()},
					In: &secp256k1fx.TransferInput{
						Amt: startBalance,
						Input: secp256k1fx.Input{
							SigIndices: []uint32{
...
func TestTxVerifyAfterVerifyAncestorTx(t *testing.T) {
	...
	secondTx := issueTxs[2]
	key := keys[0]
	firstTxDescendant := &txs.Tx{Unsigned: &txs.BaseTx{BaseTx: cflt.BaseTx{
		NetworkID:    networkID,
		BlockchainID: chainID,
		Ins: []*cflt.TransferableInput{{
			UTXOID: cflt.UTXOID{
...
func TestImportTxSerialization(t *testing.T) {
	...
	tx := &txs.Tx{Unsigned: &txs.ImportTx{
		...
		ImportedIns: []*cflt.TransferableInput{{
			UTXOID: cflt.UTXOID{TxID: ids.ID{
				0x0f, 0x2f, 0x4f, 0x6f, 0x8e, 0xae, 0xce, 0xee,
				0x0d, 0x2d, 0x4d, 0x6d, 0x8c, 0xac, 0xcc, 0xec,
				0x0b, 0x2b, 0x4b, 0x6b, 0x8a, 0xaa, 0xca, 0xea,
				0x09, 0x29, 0x49, 0x69, 0x88, 0xa8, 0xc8, 0xe8,
			}},
			Asset: cflt.Asset{ID: ids.ID{
				0x1f, 0x3f, 0x5f, 0x7f, 0x9e, 0xbe, 0xde, 0xfe,
				0x1d, 0x3d, 0x5d, 0x7d, 0x9c, 0xbc, 0xdc, 0xfc,
				0x1b, 0x3b, 0x5b, 0x7b, 0x9a, 0xba, 0xda, 0xfa,
				0x19, 0x39, 0x59, 0x79, 0x98, 0xb8, 0xd8, 0xf8,
...
func TestIssueImportTx(t *testing.T) {
	...
	txAssetID := cflt.Asset{ID: cfltID}
	tx := &txs.Tx{Unsigned: &txs.ImportTx{
		BaseTx: txs.BaseTx{BaseTx: cflt.BaseTx{
			NetworkID:    networkID,
			BlockchainID: chainID,
	...
	assertLatestIdx(t, vm.db, key.PublicKey().Address(), cfltID, 1)
...
func TestForceAcceptImportTx(t *testing.T) {
	...
	tx := &txs.Tx{Unsigned: &txs.ImportTx{
		...
		ImportedIns: []*cflt.TransferableInput{{
			UTXOID: utxoID,
			Asset:  cflt.Asset{ID: genesisTx.ID()},
			In: &secp256k1fx.TransferInput{
				Amt:   1000,
				Input: secp256k1fx.Input{SigIndices: []uint32{0}},
			},
