...
import (
	...
	"github.com/coinflect/coinflectchain/vms/components/index"
...
func setup(t *testing.T, isCFLTAsset bool) ([]byte, *VM, *Service, *atomic.Memory, *txs.Tx) {
	...
	if isCFLTAsset {
		...
		genesisTx = GetCFLTTxFromGenesisTest(genesisBytes, t)
	} else {
...
func setupWithIssuer(t *testing.T, isCFLTAsset bool) ([]byte, *VM, *Service, chan common.Message) {
	var genesisBytes []byte
	var vm *VM
...
// 3) The service that wraps the VM
// 4) atomic memory to use in tests
func setupWithKeys(t *testing.T, isCFLTAsset bool) ([]byte, *VM, *Service, *atomic.Memory, *txs.Tx) {
	genesisBytes, vm, s, m, tx := setup(t, isCFLTAsset)
...
func TestServiceGetAllBalances(t *testing.T) {
	...
	otherAssetUTXO := &cflt.UTXO{
		UTXOID: cflt.UTXOID{
			TxID:        ids.GenerateTestID(),
			OutputIndex: 0,
		},
		Asset: cflt.Asset{ID: otherAssetID},
		Out: &secp256k1fx.TransferOutput{
			Amt: 1337,
			OutputOwners: secp256k1fx.OutputOwners{
				Threshold: 2,
...
func TestServiceGetTxJSON_BaseTx(t *testing.T) {
	...
	newTx := newCfltBaseTxWithOutputs(t, genesisBytes, vm)
...
func TestServiceGetTxJSON_ExportTx(t *testing.T) {
	...
	newTx := newCfltExportTxWithOutputs(t, genesisBytes, vm)
...
func TestServiceGetTxJSON_CreateAssetTx(t *testing.T) {
	...
	createAssetTx := newCfltCreateAssetTxWithOutputs(t, vm)
...
func newCfltBaseTxWithOutputs(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	cfltTx := GetCFLTTxFromGenesisTest(genesisBytes, t)
	key := keys[0]
	tx := buildBaseTx(cfltTx, vm, key)
...
func newCfltExportTxWithOutputs(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	cfltTx := GetCFLTTxFromGenesisTest(genesisBytes, t)
	key := keys[0]
	tx := buildExportTx(cfltTx, vm, key)
	if err := tx.SignSECP256K1Fx(vm.parser.Codec(), [][]*crypto.PrivateKeySECP256K1R{{key}}); err != nil {
		t.Fatal(err)
	}
	return tx
...
func newCfltCreateAssetTxWithOutputs(t *testing.T, vm *VM) *txs.Tx {
	key := keys[0]
...
func buildBaseTx(cfltTx *txs.Tx, vm *VM, key *crypto.PrivateKeySECP256K1R) *txs.Tx {
...
func buildExportTx(cfltTx *txs.Tx, vm *VM, key *crypto.PrivateKeySECP256K1R) *txs.Tx {
	return &txs.Tx{Unsigned: &txs.ExportTx{
		...
		ExportedOuts: []*cflt.TransferableOutput{{
			Asset: cflt.Asset{ID: cfltTx.ID()},
			Out: &secp256k1fx.TransferOutput{
				Amt: startBalance - vm.TxFee,
				OutputOwners: secp256k1fx.OutputOwners{
...
func buildNFTxMintOp(createAssetTx *txs.Tx, key *crypto.PrivateKeySECP256K1R, outputIndex, groupID uint32) *txs.Operation {
	return &txs.Operation{
		Asset: cflt.Asset{ID: createAssetTx.ID()},
		UTXOIDs: []*cflt.UTXOID{{
...
func TestCreateFixedCapAsset(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, vm, s, _, _ := setupWithKeys(t, tc.cfltAsset)
			defer func() {
				if err := vm.Shutdown(context.Background()); err != nil {
					t.Fatal(err)
				}
...
func TestSendMultiple(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, vm, s, _, genesisTx := setupWithKeys(t, tc.cfltAsset)
			defer func() {
				if err := vm.Shutdown(context.Background()); err != nil {
					t.Fatal(err)
				}
...
func TestImport(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, vm, s, m, genesisTx := setupWithKeys(t, tc.cfltAsset)
			defer func() {
				if err := vm.Shutdown(context.Background()); err != nil {
					t.Fatal(err)
				}
			...
			utxo := &cflt.UTXO{
				UTXOID: cflt.UTXOID{TxID: ids.Empty},
				Asset:  cflt.Asset{ID: assetID},
				Out: &secp256k1fx.TransferOutput{
					Amt: 7,
					OutputOwners: secp256k1fx.OutputOwners{
			...
			if err := s.Import(nil, args, reply); err != nil {
				t.Fatalf("Failed to import CFLT due to %s", err)
			}
		})
	}
}
