...
func TestBaseTxSemanticVerifyUnknownFx(t *testing.T) {
	...
	tx := &txs.Tx{
		...
		Creds: []*fxs.FxCredential{{
			Verifiable: &cflt.TestVerifiable{},
		}},
	}
...
func TestBaseTxSemanticVerifyUnauthorizedFx(t *testing.T) {
	...
	fx.InitializeF = func(vmIntf interface{}) error {
		vm := vmIntf.(secp256k1fx.VM)
		return vm.CodecRegistry().RegisterType(&cflt.TestTransferable{})
	}
		...
		BaseTx: txs.BaseTx{BaseTx: cflt.BaseTx{
			...
			Ins: []*cflt.TransferableInput{
				...
				},
				{
					...
					Asset: cflt.Asset{ID: assetID}, // This asset doesn't exist
					In: &secp256k1fx.TransferInput{
						Amt:   startBalance,
						Input: secp256k1fx.Input{SigIndices: []uint32{0}},
					},
...
func TestExportTxSemanticVerifyInvalidFx(t *testing.T) {
	...
	utx.Tx.Creds[0].Verifiable = &cflt.TestVerifiable{}
	err = rawTx.Unsigned.Visit(&txSemanticVerify{
		tx: utx.Tx,
		vm: vm,
	})
...
func TestExportTxSemanticVerifyTransferCustomAsset(t *testing.T) {
	...
	genesisCfltTx := GetCFLTTxFromGenesisTest(genesisBytes, t)
	cfltID := genesisCfltTx.ID()
