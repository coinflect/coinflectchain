...
import (
	...
	"github.com/coinflect/coinflectchain/pubsub"
	...
	extensions "github.com/coinflect/coinflectchain/vms/avm/fxs"
)
...
func (vm *VM) Initialize(
	...
	vm.AddressManager = cflt.NewAddressManager(ctx)
	...
	vm.AtomicUTXOManager = cflt.NewAtomicUTXOManager(ctx.SharedMemory, vm.parser.Codec())
...
func (vm *VM) initGenesis(genesisBytes []byte) error {
	...
	// secure this by defaulting to cfltAsset
	vm.feeAssetID = vm.ctx.CFLTAssetID
...
func (vm *VM) getUTXO(utxoID *cflt.UTXOID) (*cflt.UTXO, error) {
...
func (vm *VM) verifyTransferOfUTXO(utx txs.UnsignedTx, in *cflt.TransferableInput, cred verify.Verifiable, utxo *cflt.UTXO) error {
...
func (vm *VM) verifyTransfer(tx txs.UnsignedTx, in *cflt.TransferableInput, cred verify.Verifiable) error {
...
func (vm *VM) LoadUser(
	...
	*secp256k1fx.Keychain,
	...
	utxos, err := cflt.GetAllUTXOs(vm.state, kc.Addresses())
	if err != nil {
...
func (vm *VM) Spend(
	...
	cflt.SortTransferableInputsWithSigners(ins, keys)
...
func (vm *VM) SpendNFT(
	...
	for _, utxo := range utxos {
		...
		ops = append(ops, &txs.Operation{
			Asset:   utxo.Asset,
			UTXOIDs: []*cflt.UTXOID{&utxo.UTXOID},
...
func (vm *VM) selectChangeAddr(defaultAddr ids.ShortID, changeAddr string) (ids.ShortID, error) {
	...
	addr, err := cflt.ParseServiceAddress(vm, changeAddr)
	if err != nil {
