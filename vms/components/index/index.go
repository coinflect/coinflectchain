...
type AddressTxsIndexer interface {
	...
	Accept(
		txID ids.ID,
		inputUTXOs []*cflt.UTXO,
		outputUTXOs []*cflt.UTXO,
	) error
...
func (i *indexer) Accept(txID ids.ID, inputUTXOs []*cflt.UTXO, outputUTXOs []*cflt.UTXO) error {
	utxos := inputUTXOs
	...
	for _, utxo := range utxos {
		out, ok := utxo.Out.(cflt.Addressable)
		if !ok {
...
func (*noIndexer) Accept(ids.ID, []*cflt.UTXO, []*cflt.UTXO) error {
	return nil
}
