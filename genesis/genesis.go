...
import (
	...
	"github.com/coinflect/coinflectchain/vms/platformvm/genesis"
	"github.com/coinflect/coinflectchain/vms/propertyfx"
	"github.com/coinflect/coinflectchain/vms/secp256k1fx"

	xchaintxs "github.com/coinflect/coinflectchain/vms/avm/txs"
	pchaintxs "github.com/coinflect/coinflectchain/vms/platformvm/txs"
)
...
func validateInitialStakedFunds(config *Config) error {
	...
	for _, allocation := range config.Allocations {
		...
		// ethAddrs could claim to the same cfltAddr.
		allocationSet.Add(allocation.CFLTAddr)
	}

	for _, staker := range config.InitialStakedFunds {
		if initialStakedFundsSet.Contains(staker) {
			cfltAddr, err := address.Format(
				configChainIDAlias,
...
//
// Since an Coinflect network has exactly one Platform Chain, and the Platform
...
func FromConfig(config *Config) ([]byte, ids.ID, error) {
	...
	}
	{
		...
		for _, allocation := range xAllocations {
			addr, err := address.FormatBech32(hrp, allocation.CFLTAddr.Bytes())
			if err != nil {
				return nil, ids.ID{}, err
			}

			cflt.InitialState["fixedCap"] = append(cflt.InitialState["fixedCap"], avm.Holder{
		...
		var err error
		cflt.Memo, err = formatting.Encode(defaultEncoding, memoBytes)
		if err != nil {
			return nil, ids.Empty, fmt.Errorf("couldn't parse memo bytes to string: %w", err)
		}
		avmArgs.GenesisData = map[string]avm.AssetDefinition{
			"CFLT": cflt, // The AVM starts out with one asset: CFLT
		}
	}
	...
	cfltAssetID, err := CFLTAssetID(bytes)
	if err != nil {
		return nil, ids.ID{}, fmt.Errorf("couldn't generate CFLT asset ID: %w", err)
	}
	...
	for _, allocation := range config.Allocations {
		if initiallyStaked.Contains(allocation.CFLTAddr) {
	...
	return genesisBytes, cfltAssetID, nil
}
...
func CFLTAssetID(avmGenesisBytes []byte) (ids.ID, error) {
...
func (xa innerSortXAllocation) Less(i, j int) bool {
	return xa[i].InitialAmount < xa[j].InitialAmount ||
		(xa[i].InitialAmount == xa[j].InitialAmount &&
			bytes.Compare(xa[i].CFLTAddr.Bytes(), xa[j].CFLTAddr.Bytes()) == -1)
}
