...
type StakingConfig struct {
	...
	// Minimum stake, in nCFLT, required to validate the primary network
	MinValidatorStake uint64 `json:"minValidatorStake"`
	// Maximum stake, in nCFLT, allowed to be placed on a single validator in
	// the primary network
...
func GetTxFeeConfig(networkID uint32) TxFeeConfig {
	...
	case constants.AddismyaID:
		return AddismyaParams.TxFeeConfig
...
func GetStakingConfig(networkID uint32) StakingConfig {
	...
	case constants.AddismyaID:
		return AddismyaParams.StakingConfig
	case constants.LocalID:
