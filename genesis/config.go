...
type Allocation struct {
	...
	CFLTAddr       ids.ShortID    `json:"cfltAddr"`
...
func (c Config) Unparse() (UnparsedConfig, error) {
	...
	for i, isa := range c.InitialStakedFunds {
		...
		uc.InitialStakedFunds[i] = cfltAddr
	}
...
var (
	...
	// AddismyaConfig is the config that should be used to generate the addismya
	// genesis.
	AddismyaConfig Config
...
func init() {
	...
	unparsedAddismyaConfig := UnparsedConfig{}
	unparsedLocalConfig := UnparsedConfig{}

	errs := wrappers.Errs{}
	errs.Add(
		json.Unmarshal(mainnetGenesisConfigJSON, &unparsedMainnetConfig),
		json.Unmarshal(addismyaGenesisConfigJSON, &unparsedAddismyaConfig),
	...
	addismyaConfig, err := unparsedAddismyaConfig.Parse()
	errs.Add(err)
	AddismyaConfig = addismyaConfig
