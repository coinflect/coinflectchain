...
func TestGenesisFromFile(t *testing.T) {
	tests := map[string]struct {
		...
		"addismya": {
			networkID:    constants.AddismyaID,
			customConfig: customGenesisConfigJSON,
			err:          "cannot override genesis config for standard network addismya (5)",
		},
		"addismya (with custom specified)": {
			networkID:    constants.AddismyaID,
		...
		t.Run(name, func(t *testing.T) {
			...
			if len(test.customConfig) == 0 {
				...
				case constants.TestnetID:
					genBytes, err = json.Marshal(&AddismyaConfig)
					require.NoError(err)
...
func TestCFLTAssetID(t *testing.T) {
	tests := []struct {
		networkID  uint32
		expectedID string
	}{
	...
	for _, test := range tests {
		t.Run(constants.NetworkIDToNetworkName[test.networkID], func(t *testing.T) {
			...
			config := GetConfig(test.networkID)
			_, cfltAssetID, err := FromConfig(config)
			require.NoError(err)

			require.Equal(
				test.expectedID,
				cfltAssetID.String(),
				"CFLT assetID with networkID %d mismatch",
				test.networkID,
			)
		})
	}
