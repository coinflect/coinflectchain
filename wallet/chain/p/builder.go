...
func (b *builder) NewBaseTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.CFLTAssetID(): b.backend.CreateSubnetTxFee(),
	}
	...
	cflt.SortTransferableOutputs(outputs, txs.Codec) // sort the outputs

	return &txs.CreateSubnetTx{
		BaseTx: txs.BaseTx{BaseTx: cflt.BaseTx{
...
func (b *builder) NewAddValidatorTx(
	...
	cfltAssetID := b.backend.CFLTAssetID()
	toBurn := map[ids.ID]uint64{
		cfltAssetID: b.backend.AddPrimaryNetworkValidatorFee(),
	}
	toStake := map[ids.ID]uint64{
		cfltAssetID: vdr.Wght,
	}
	ops := common.NewOptions(options)
...
func (b *builder) NewAddSubnetValidatorTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.CFLTAssetID(): b.backend.AddSubnetValidatorFee(),
	}
	toStake := map[ids.ID]uint64{}
	ops := common.NewOptions(options)
...
func (b *builder) NewRemoveSubnetValidatorTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.CFLTAssetID(): b.backend.BaseTxFee(),
	}
	toStake := map[ids.ID]uint64{}
	ops := common.NewOptions(options)
	inputs, outputs, _, err := b.spend(toBurn, toStake, ops)
...
func (b *builder) NewAddDelegatorTx(
	...
	cfltAssetID := b.backend.CFLTAssetID()
	toBurn := map[ids.ID]uint64{
		cfltAssetID: b.backend.AddPrimaryNetworkDelegatorFee(),
	}
	toStake := map[ids.ID]uint64{
		b.backend.CFLTAssetID(): vdr.Wght,
	}
	ops := common.NewOptions(options)
	inputs, baseOutputs, stakeOutputs, err := b.spend(toBurn, toStake, ops)
	if err != nil {
...
func (b *builder) NewCreateChainTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.CFLTAssetID(): b.backend.CreateBlockchainTxFee(),
	}
	toStake := map[ids.ID]uint64{}
	ops := common.NewOptions(options)
	inputs, outputs, _, err := b.spend(toBurn, toStake, ops)
...
func (b *builder) NewImportTx(
	...
	var (
		...
		importedInputs  = make([]*cflt.TransferableInput, 0, len(utxos))
	...
	cflt.SortTransferableInputs(importedInputs) // sort imported inputs
	...
	var (
		...
		outputs      = make([]*cflt.TransferableOutput, 0, len(importedAmounts))
		importedCFLT = importedAmounts[cfltAssetID]
	)
	if importedCFLT > txFee {
		importedAmounts[cfltAssetID] -= txFee
	} else {
		if importedCFLT < txFee { // imported amount goes toward paying tx fee
			toBurn := map[ids.ID]uint64{
				cfltAssetID: txFee - importedCFLT,
			}
			toStake := map[ids.ID]uint64{}
			var err error
			inputs, outputs, _, err = b.spend(toBurn, toStake, ops)
		...
		delete(importedAmounts, cfltAssetID)
	}

	for assetID, amount := range importedAmounts {
		outputs = append(outputs, &cflt.TransferableOutput{
	...
	cflt.SortTransferableOutputs(outputs, txs.Codec) // sort imported outputs
	return &txs.ImportTx{
		BaseTx: txs.BaseTx{BaseTx: cflt.BaseTx{
			NetworkID:    b.backend.NetworkID(),
			BlockchainID: constants.PlatformChainID,
...
func (b *builder) NewExportTx(
	...
	cflt.SortTransferableOutputs(outputs, txs.Codec) // sort exported outputs
	return &txs.ExportTx{
		BaseTx: txs.BaseTx{BaseTx: cflt.BaseTx{
			NetworkID:    b.backend.NetworkID(),
			BlockchainID: constants.PlatformChainID,
...
func (b *builder) NewTransformSubnetTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.CFLTAssetID(): b.backend.TransformSubnetTxFee(),
...
func (b *builder) NewAddPermissionlessValidatorTx(
	...
	if vdr.Subnet == constants.PrimaryNetworkID {
		toBurn[cfltAssetID] = b.backend.AddPrimaryNetworkValidatorFee()
	} else {
		toBurn[cfltAssetID] = b.backend.AddSubnetValidatorFee()
	}
	toStake := map[ids.ID]uint64{
		assetID: vdr.Wght,
	}
...
func (b *builder) NewAddPermissionlessDelegatorTx(
	...
	toBurn := map[ids.ID]uint64{}
	if vdr.Subnet == constants.PrimaryNetworkID {
		toBurn[cfltAssetID] = b.backend.AddPrimaryNetworkDelegatorFee()
	} else {
		toBurn[cfltAssetID] = b.backend.AddSubnetDelegatorFee()
	}
	toStake := map[ids.ID]uint64{
		assetID: vdr.Wght,
	}
...
func (b *builder) spend(
	...
	inputs []*cflt.TransferableInput,
	changeOutputs []*cflt.TransferableOutput,
	stakeOutputs []*cflt.TransferableOutput,
	err error,
) {
	...
	for _, utxo := range utxos {
		...
		inputs = append(inputs, &cflt.TransferableInput{
			UTXOID: utxo.UTXOID,
			Asset:  utxo.Asset,
			In: &stakeable.LockIn{
		...
		stakeOutputs = append(stakeOutputs, &cflt.TransferableOutput{
			Asset: utxo.Asset,
		...
		if remainingAmount := out.Amt - amountToStake; remainingAmount > 0 {
			...
			changeOutputs = append(changeOutputs, &cflt.TransferableOutput{
				Asset: utxo.Asset,
				Out: &stakeable.LockOut{
					Locktime: lockedOut.Locktime,
					TransferableOut: &secp256k1fx.TransferOutput{
	...
	cflt.SortTransferableInputs(inputs)                    // sort inputs
	cflt.SortTransferableOutputs(changeOutputs, txs.Codec) // sort the change outputs
	cflt.SortTransferableOutputs(stakeOutputs, txs.Codec)  // sort stake outputs
